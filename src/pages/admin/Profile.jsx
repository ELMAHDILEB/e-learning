...
import { Camera, Mail, Phone, MapPin, Globe, Lock, Bell, Shield, Save } from "lucide-react";
...
const Profile = () => {
  ...
  const [personal, setPersonal] = useState({
    firstName: "Ahmed",
    lastName: "Benali",
    email: "ahmed@learnova.com",
    phone: "+212 6 12 34 56 78",
    bio: "Senior administrator at Learnova. Passionate about e-learning and education technology.",
    location: "Casablanca, Morocco",
    website: "https://learnova.com",
    role: "Admin",
  });
    ...
    <div className="flex flex-col gap-6 max-w-4xl">
        ...
        <div className="flex flex-wrap items-center gap-5">
          <div className="relative">
            <div className="w-20 h-20 rounded-full bg-cyan-500 flex items-center justify-center text-white text-2xl font-semibold overflow-hidden">
              {avatar ? <img src={avatar} alt="avatar" className="w-full h-full object-cover" /> : "AB"}
            </div>
          ...
          <div className="flex-1">
            <h2 className="text-base font-semibold">{personal.firstName} {personal.lastName}</h2>
            <p className="text-sm text-[var(--text)] opacity-50">{personal.role} · {personal.location}</p>
            <p className="text-xs text-[var(--text)] opacity-40 mt-1">{personal.email}</p>
          </div>
      ...
      {activeTab === "Personal Info" && (
        <div className="bg-[var(--card)] rounded-xl border border-[var(--border)] p-5 flex flex-col gap-4">
          <h3 className="text-sm font-medium flex items-center gap-2"><Shield size={15} className="text-cyan-500" /> Personal Information</h3>
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs text-[var(--text)] opacity-50 flex items-center gap-1"><Mail size={11} /> Email</label>
            <input
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs text-[var(--text)] opacity-50 flex items-center gap-1"><Phone size={11} /> Phone</label>
            <input
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              <label className="text-xs text-[var(--text)] opacity-50 flex items-center gap-1"><MapPin size={11} /> Location</label>
              <input
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-[var(--text)] opacity-50 flex items-center gap-1"><Globe size={11} /> Website</label>
              <input
      ...
      {activeTab === "Security" && (
        <div className="bg-[var(--card)] rounded-xl border border-[var(--border)] p-5 flex flex-col gap-4">
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-3">
            <p className="text-xs text-yellow-500">Password must be at least 8 characters, include uppercase, lowercase, and a number.</p>
          </div>
      ...
      {activeTab === "Notifications" && (
        <div className="bg-[var(--card)] rounded-xl border border-[var(--border)] p-5 flex flex-col gap-4">
          ...
          {[
            ["newUser", "New User Registration", "Get notified when a new user signs up"],
            ["newCourse", "New Course Submitted", "Get notified when an instructor submits a course"],
            ["coursePublished", "Course Published", "Get notified when a course goes live"],
            ["weeklyReport", "Weekly Report", "Receive a weekly summary of platform activity"],
            ["systemAlerts", "System Alerts", "Critical system and security alerts"],
            ["emailDigest", "Email Digest", "Daily email digest of all activity"],
          ].map(([key, label, desc]) => (
      ...
      {activeTab === "Preferences" && (
        <div className="bg-[var(--card)] rounded-xl border border-[var(--border)] p-5 flex flex-col gap-4">
          <h3 className="text-sm font-medium flex items-center gap-2"><Globe size={15} className="text-cyan-500" /> Platform Preferences</h3>
          {[
            ["language", "Language", ["English", "Arabic", "French", "Spanish"]],
            ["timezone", "Timezone", ["Africa/Casablanca", "Europe/Paris", "America/New_York", "Asia/Dubai"]],
