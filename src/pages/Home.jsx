...
const Home = () => {
  return (
    <main>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 py-20 md:py-28 relative">
          <div className="max-w-3xl animate-fade-up motion-reduce:animate-none">
            ...
            <div className="mt-8 flex flex-wrap gap-4 animation-delay-200 animate-fade-up-sm motion-reduce:animate-none opacity-0 [animation-fill-mode:forwards]">
              <Link
                to="/register"
      ...
      {/* Stats */}
      <AnimatedSection as="section" className="border-y border-[var(--border)] bg-[var(--card)]">
        <div className="max-w-7xl mx-auto px-4 py-12">
          <Stagger className="grid grid-cols-2 md:grid-cols-4 gap-8" step={80}>
            {stats.map((stat) => (
      ...
      <section className="max-w-7xl mx-auto px-4 py-20">
        ...
        <Stagger className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" baseDelay={100} step={70}>
          {features.map(({ icon: Icon, title, description }) => (
            <div
              key={title}
              className="ui-card p-6 rounded-xl bg-[var(--card)] border border-[var(--border)]"
            >
      ...
      <AnimatedSection as="section" className="bg-[var(--card)] border-y border-[var(--border)]">
        <div className="max-w-7xl mx-auto px-4 py-20">
          ...
          <Stagger className="grid grid-cols-1 md:grid-cols-3 gap-8" step={100}>
        ...
        <AnimatedSection>
          <div className="rounded-2xl bg-gradient-to-r from-cyan-500 to-cyan-600 p-10 md:p-14 text-center text-white">
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-4">
              <Link
                to="/register"
                className="ui-btn inline-flex items-center gap-2 px-6 py-3 rounded-md bg-white text-cyan-600 font-medium hover:bg-white/90"
              >
                Create Free Account
                <ArrowRight size={18} />
              </Link>
              <Link
                to="/contact"
                className="ui-btn inline-flex items-center gap-2 px-6 py-3 rounded-md border border-white/40 font-medium hover:bg-white/10"
              >
                Contact Us
              </Link>
            </div>
